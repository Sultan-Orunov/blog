                ...
                <div class="col-lg-8">
                    @foreach($posts as $post)
                        <div class="blog-entry d-flex blog-entry-search-item">
                            ...
                            <div>
                                <span class="date">{{ $post->created_at->diffForHumans() }} &bullet; <a href="{{ route('category.posts', $post->category->id) }}">{{ ucfirst($post->category->title) }}</a></span>
                    ...
                    <div class="sidebar-box">
                        ...
                        <ul class="categories">
                            @foreach($categories as $category)
                                <li><a class="text-decoration-none text-secondary" href="{{ route("category.posts", $category->id) }}">{{ ucfirst($category->title) }}<span>{{count($category->posts)}}</span></a></li>
                            @endforeach
                        </ul>
                    </div>
                    <!-- END sidebar-box -->
